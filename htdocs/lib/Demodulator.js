...
{
	var max_bw = (audioEngine.getOutputRate() / 2) - 1;

	if (['pocsag', 'page', 'packet', 'ais', 'acars'].indexOf(this.demodulator.get_secondary_demod()) >= 0)
//		max_bw = 12500;
		max_bw = 23500;      // HLR
	else
	if (['dmr', 'dstar', 'nxdn', 'ysf', 'm17'].indexOf(this.demodulator.get_modulation()) >= 0)
		max_bw = 6250;
	else
	if (['nfm', 'fm', 'am', 'sam'].indexOf(this.demodulator.get_modulation()) >= 0)    // HLR
		max_bw = 23500;
	else
	if (this.demodulator.get_modulation() === 'wfm')
		max_bw = 100000;
	else
	if (this.demodulator.get_modulation() === 'drm')
		max_bw = 50000;
	else
	if (this.demodulator.get_modulation() === "freedv")
		max_bw = 4000;
	else
	if (this.demodulator.get_secondary_demod() === "ism")
		max_bw = 600000;
	
	return {high: max_bw, low: -max_bw};
};
...
Envelope.prototype.draw = function(visible_range){
    ...
    var env_bounding_line_w = 4;   // HLR
    var env_att_w = 5;             //     _______   ___env_h2 in px   ___|_____               
    var env_h1 = 17;               //   _/|      \_ ___env_h1 in px _/   |_    \_
    var env_h2 = 8;                // HLR  |||env_att_line_w                |_env_lineplus
    var env_lineplus = 2;          // HLR  ||env_bounding_line_w
    ...
    if (!(to_px < 0 || from_px > window.innerWidth)) // out of screen ?
    {
        ...
        scale_ctx.lineWidth   = 1;       // HLR
        scale_ctx.globalAlpha = 0;     // HLR
        scale_ctx.fill();
        ...
        //scale_ctx.lineWidth    = 1; // HLR
        //scale_ctx.font         = "normal 11px sans-serif"; // HLR
        //scale_ctx.textBaseline = "top"; // HLR

        //scale_ctx.textAlign    = "left"; // HLR
        //if (typeof(this.demodulator.high_cut) === 'number') // HLR
		{
        //    scale_ctx.fillText(this.demodulator.high_cut.toString(), to_px + env_att_w, env_h2 + 1);    // HLR
        }

        //scale_ctx.textAlign = "right"; // HLR
        //if (typeof(this.demodulator.low_cut) === 'number') // HLR
		{
          //  scale_ctx.fillText(this.demodulator.low_cut.toString(), from_px - env_att_w, env_h2 + 1);   // HLR
        }
    }
